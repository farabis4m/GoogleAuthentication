...
- (id)initWithSecret:(NSData *)secret
  ...
  if ((self = [super init])) {
    ...
    if (!goodAlgorithm || digits_ > 8 || digits_ < 6 || !secret_) {
      _GTMDevLog(@"Bad args digits(min 6, max 8): %lu secret: %@ algorithm: %@",
...
- (NSString *)generateOTPForCounter:(uint64_t)counter {
  ...
  if ([algorithm_ isEqualToString:kOTPGeneratorSHA1Algorithm]) {
    ...
    hashLength = CC_SHA512_DIGEST_LENGTH;
      ...
      // No need to handle as this algorithm is not used anymore.
      // Risk team asked to remove as case of Vulnerability
      _GTMDevAssert(NO, @"SHAMD5 Algorithm not used anymore & removed because of Vulnerability");
      return nil;
  } else {
